...
function copyFolder($source, $destination)
{
    ...
    foreach ($iterator as $item) {
        if ($item->isDir()) {
            ...
            if (substr($filename, -8) === '-vp0.jpg' || substr($filename, -8) === '-vp1.jpg' || $filename === 'placa.png' || $filename === 'felpudo.png') {
...
function comprimirCarpeta($carpeta, $archivoZip)
{
    ...
    // Intentamos abrir el archivo ZIP para escritura
    if ($zip->open($archivoZip, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
        die("No se pudo crear el archivo ZIP.");
    }

    // Recorremos la carpeta y agregamos los archivos/directorios al ZIP
    ...
    foreach ($archivos as $archivo) {
        $nombre = $archivo->getRealPath();
        $archivoEnZip = substr($nombre, strlen(realpath($carpeta)) + 1);

        if ($archivo->isFile()) {
            $zip->addFile($nombre, $archivoEnZip);
        } elseif ($archivo->isDir()) {
            $zip->addEmptyDir($archivoEnZip);
        }
    }
    ...
    echo "La carpeta ha sido comprimida correctamente.";
}
...
$felpudo= $_GET['felpudo'];


$origen = "../../data/$archivo/$felpudo";
//$salida = "../../salida/" . $archivo;

$salida = "../../salida/$archivo/$felpudo";
...
$carpeta = '../../carpeta/';
$comprimir = "../../salida/$archivo/";
//$zipPath = zipDirectoryAndCreateLink($salida, $carpeta . $archivo . '.zip');

$comprimir = comprimirCarpeta($comprimir, $carpeta . $archivo . '.zip');
...
echo "<a class='btn btn-success' href='https://chat.getsingular.com/amazon/descarga.php?file=carpeta/" . $archivo . ".zip'><i class='bi bi-arrow-down-circle'></i> Descargar</a> <br>";