...
$llaveros = $_GET['llaveros'];
 if($llaveros == '1'){
    $tipodeplaca = "Placa_Spotify_1llavero";
 }else{
    $tipodeplaca = "Placa_Spotify_2llaveros";
 }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        foreach ($archivos as $archivo) {
            if (pathinfo($archivo, PATHINFO_EXTENSION) == "json") {
                ...
                leerJSON($rutaJson, $ruta, $base,$llaveros);
...
$sql = "SELECT COUNT(*) as carpetasRestantes FROM carpetas WHERE procesada = 0";
$result = $mysqli->query($sql);
$row = $result->fetch_assoc();
$carpetasRestantes = $row['carpetasRestantes'];

$response = array(
    'message' => 'Carpeta procesada exitosamente.',
    'carpetasRestantes' => $carpetasRestantes,
...
function leerJSON($json, $ruta, $base, $llaveros)
{
    global $rootPath;
    global $pedido;
    global $datetime_str;
    ...
    registro("Comenzó el procesamiento del pedido: $pedido");
    registro("Cancion: $titulo[0]");
    registro("Artista: $artista[0]");
    registro("URL: $urlSpotify");



    grabarPlaca($titulo[0], $artista[0], $duracion, $ruta, $qr, $llaveros);
    registro("Frase de la base: $frase[0]");
    grabarMadera($frase[0], $ruta);
    //grabarMadera($frase[0], $url_completa);
    /* if ($base == "Placa_Spotify_BASE") {
        registro("Frase de la base: $frase[0]");
        grabarMadera($frase[0], $ruta);
    } */
}
...
{
    try {
        $imagenBase = new Imagick('imagenes/placa_llavero.png');
