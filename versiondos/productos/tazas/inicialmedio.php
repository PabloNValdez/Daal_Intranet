...
function tazasMedio($registros)
{
    try {
        ...
        mensaje($fuenteDos);
        mensaje($nombre);
        crearImagenInicialesMedio($inicial, $colorTexto[0], $fuente[0], $url_completa);
        crearImagenTextoMedio($nombre, $fuenteDos, $url_completa);
        crearTazasMedio($colorTaza, $url_completa,$pedido);
    } catch (Exception $e) {
...
function crearTazasMedio($colorTaza, $url_completa,$pedido)
{
    try {
        // Ruta de las imágenes
        $rutaImagenBase = 'productos/tazas/bases/textoBase.png';
        $rutaImagenSuperior = 'productos/tazas/textomedio.png';
        ...
        $rutaImagenResultante = 'productos/tazas/final_texto_medio.png';
        ...
        $rutaImagenSuperior = 'productos/tazas/inicialMedio.png';
        $x = 49; // Coordenada x
        $y = 54; // Coordenada y

        $anchoArea = 364; // Ancho del área
        $altoArea = 386; // Alto del área

        superponerImagenMedio($rutaImagenBase, $rutaImagenSuperior, $x, $y, $anchoArea, $altoArea, $colorTaza, $url_completa);
        ...
        $rutaImagenBase = 'productos/tazas/taza_final.png';
        $rutaImagenSuperior = 'productos/tazas/final_texto_medio.png';
        $x = 49; // Coordenada x
        $y = 227; // Coordenada y

        $anchoArea = 364; // Ancho del área
        $altoArea = 96; // Alto del área
        ...
        $rutaResultado = superponerImagenMedio($rutaImagenBase, $rutaImagenSuperior, $x, $y, $anchoArea, $altoArea, $colorTaza, $url_completa);
...
function crearImagenInicialesMedio($iniciales, $color, $fuente, $url_completa)
{
    ...
    $anchoArea = 772.5;
    $altoArea = 887.5;
    $rutaImagenSalida = "productos/tazas/inicialMedio.png";
    ...
    generarImagenInicialesMedio($texto, $anchoArea, $altoArea, $fuente, $rutaImagenSalida, $color, $tamanoFuenteMaximo);
}

function crearImagenTextoMedio($nombre, $fuenteDos, $url_completa)
{
    //CREAMOS TEXTO INFERIOR
    $texto = $nombre;
    $anchoArea = 618;
    $altoArea = 152;
    $rutaImagenSalida = "productos/tazas/textomedio.png";
...
function generarImagenInicialesMedio($texto, $anchoArea, $altoArea, $fuente, $rutaGuardado, $colorTexto, $tamanoFuenteMaximo)
{
    $imagen = new Imagick();
...
function superponerImagenMedio($rutaImagenBase, $rutaImagenSuperior, $x, $y, $anchoArea, $altoArea, $colorTaza)
{
    // Carga las imágenes con Imagick
    $imagenBase = new \Imagick($rutaImagenBase);
    $imagenSuperior = new \Imagick($rutaImagenSuperior);
