...
function nuevasPlacas($registros)
{
    ...
    $outputPath = 'temp/'.$pedido.'-image.png';
    ...
    if ($variantes === "Placa_Spotify_BASE" || $asin === "B0C122G7Y7") {
        grabarPlacaSimpleNueva($titulo, $artista, $duracion, $url_completa, $qr, $pedido);
        mensaje("Frase de la base: $frase[0]");
        grabarMaderaNueva(@$frase[0], @$url_completa);
    } elseif ($variantes === "Placa_Spotify_sin_BASE") {
        grabarPlacaSimpleNueva($titulo, $artista, $duracion, $url_completa, $qr, $pedido, $variantes);
    } elseif ($variantes === "_Placa_Spotify_1llavero" || $variantes === "Placa_Spotify_1llavero") {
        $llaveros = '1';
        grabarPlacaLlaveroNueva($titulo, $artista, $duracion, $url_completa, $qr, $pedido, $llaveros);
        mensaje("Frase de la base: $frase[0]");
        grabarMaderaNueva(@$frase[0], @$url_completa);
    } elseif ($variantes === "Placa_Spotify_Luz") {
        placaSpotifyLuz($titulo, $artista, $duracion, $url_completa, $qr, $llaveros, $tipoDePlaca,$pedido);
        mensaje("Frase de la base: $frase[0]");
        if ($tipoDePlaca != "Sin Base") {
            grabarMaderaNueva(@$frase[0], @$url_completa);
        }
    } else {
        $llaveros = '0';
...
function placaSpotifyLuz($cancion, $artista, $duracion, $url_completa, $qr, $llaveros, $tipoDePlaca,$pedido)
{
    try {
        mensaje("$cancion");
        mensaje("$artista");
        ...
        if ($tipoDePlaca == "Base con luz 7 colores") {
            ...
            if ($qr !== 0) {
                $imagenQR = new Imagick('productos/test-placas/QRDescargado/'.$pedido.'-imagen_transparente.png');
                $llaveroUno = new Imagick('productos/test-placas/QRDescargado/'.$pedido.'-imagen_transparente.png');
        ...
        } elseif ($tipoDePlaca == "Base de madera" || $tipoDePlaca == "Sin Base") {
            ...
            if ($llaveros === "2 Llavero") { //DOS LLAVEROS SIN LUZ LED
                if($tipoDePlaca == "Base de madera"){
                ...
                $imagenBase->compositeImage($llaveroUno, Imagick::COMPOSITE_DEFAULT, 890, 935);
                $imagenBase->compositeImage($llaveroUno, Imagick::COMPOSITE_DEFAULT, 890, 550);
            } elseif ($llaveros === "1 Llavero") { //UN LLAVEROS SIN LUZ LED
                if($tipoDePlaca == "Base de madera"){
        ...
        $imagenSuperior = new Imagick('temp/'.$pedido.'-image.png');
...
function grabarPlacaSimpleNueva($cancion, $artista, $duracion, $url_completa, $qr,$pedido)
{
    try {
        ...
        $ruta_completa = $url_completa . $nombre_archivo;
        $imagenBase->writeImage($ruta_completa);
        mensaje("Placa creada correctamente");
...
function grabarPlacaLlaveroNueva($cancion, $artista, $duracion, $url_completa, $qr, $pedido, $llaveros)
{
        mensaje("$cancion");
        mensaje("$artista");
        mensaje("$duracion");
    ...
    try {
        mensaje("Inicio de procesamiento de: Placa_Spotify_1llavero");
...
function ObtenerQRNueva($trackId, $color, $pedido)
{
    ...
    $dir = 'productos/test-placas/QRDescargado';
    if (!file_exists($dir)) {
    ...
    try {
        ...
        mensaje("QR: ".$dir . '/'.$pedido.'-imagen_transparente.png');
